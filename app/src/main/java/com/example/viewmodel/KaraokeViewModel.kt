    ...
    private fun writeBitmapToImage(bitmap: Bitmap, image: android.media.Image, argbBuffer: IntArray) {
        ...
        for (row in 0 until height) {
            ...
            for (col in 0 until width) {
                ...
                yBuffer.put(yRowStart + col * yPixelStride, (if (y < 0) 0 else if (y > 255) 255 else y).toByte())
            }
        }
        ...
        for (row in 0 until height step 2) {
            ...
            for (col in 0 until width step 2) {
                ...
                uBuffer.put(uRowStart + (col / 2) * uPixelStride, (if (u < 0) 0 else if (u > 255) 255 else u).toByte())
                vBuffer.put(vRowStart + (col / 2) * vPixelStride, (if (v < 0) 0 else if (v > 255) 255 else v).toByte())
            }
        }
    }
